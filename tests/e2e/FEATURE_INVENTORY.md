...
| `save_memory` tool | `core/tools/memory_tools.py:22-61` | Persist a memory record. | (1) LLM decides to remember; (2) /memory command reads it back. | Invalid memory_type or scope (returns error string). | (indirect) | `test_middleware_ordering_e2e.py::test_save_memory_tool_persists_before_stop_hook_runs`. | Invalid args at e2e; conflict on duplicate title. | inventoried |
| `list_memories` tool | `core/tools/memory_tools.py:63-` | List stored memories by scope/type. | (1) User asks what's remembered; (2) Filter by type. | Empty store; invalid scope. | (indirect) | None. | Empty-result e2e; scope filter e2e. | inventoried |
...
| `update_memory` / `delete_memory` tools | `core/tools/memory_tools.py` | CRUD mutations. | (1) Correct old facts; (2) Drop stale entries. | Unknown id; wrong scope. | (indirect) | None. | Round-trip e2e. | inventoried |
| `retrieve_result` tool | `core/graph_builder/tools.py:86-94` | Pull persisted large tool outputs back into context on demand. | (1) Fetch a truncated prior result; (2) Dispatch from LLM. | Unknown key; expired. | (indirect) | None. | End-to-end: persist, truncate, retrieve. | inventoried |
| Skill tools (`list_skills`, `read_skill`) | `core/skills/tools.py` | Progressive-disclosure skill discovery. | (1) Discover available skills; (2) Load full instructions on demand. | No skills registered; skill path missing. | (indirect) | Phase 1 spike used list_skills. | Full "skill activation" flow e2e. | inventoried |
| Async task tools | `core/orchestration/workers.py` | Fire-and-forget background agents. | (1) `start_async_task`; (2) `check_async_task`; (3) `cancel_async_task`; (4) `list_async_tasks`. | Unknown task id; cancellation races; completion after parent exit. | `test_integration_features.py` | None. | Round-trip e2e. | inventoried |
| UI tools | `core/ui_events.py` + `core/artifacts.py` | `create_artifact`, `emit_progress`, `suggest_actions`, `add_citation`. | (1) Model emits artifact; (2) Progress pings UI; (3) Suggested next actions shown; (4) Citation added to source. | Malformed artifact payload; very large content. | `test_integration_features.py` | None. | Each UI tool happy path. | inventoried |
| HITL tools | `core/hitl/` | `approve_action` — request human approval before mutating operations. | (1) LLM gates a destructive action; (2) User approves / denies. | Denial path; timeout. | `test_integration_features.py` (indirect) | None. | Approval round-trip. | inventoried |
...
| `RuntimeStateMiddleware` | `core/resilience/runtime_state.py` | Initializes state bookkeeping. | (1) Fresh run init; (2) Retry tracking. | — | `test_reference_deep_agent.py` | (indirect) | — | inventoried |
| `QueuedInputMiddleware` | `core/orchestration/` | Queued inputs from async subagents appear in the turn. | (1) Worker completes mid-conversation; (2) Queue empty. | Worker failure. | `test_queue.py` | None. | Queue-delivered input reaches LLM e2e. | inventoried |
| `ToolErrorMiddleware` | `core/resilience/` | Retries failing tool calls up to N times. | (1) Transient error retried; (2) Permanent error surfaces. | `max_retries=0`; exception types. | `test_resilience.py` | None. | Retry observed in state e2e. | inventoried |
...
| `PressureMiddleware` | `core/context_management/pressure_middleware.py` | Monitors window pressure, triggers compaction. | (1) Below threshold passthrough; (2) Above threshold compacts. | Monitor None; empty messages. | `test_context_management.py`, `test_pressure_full_compaction.py` | None. | Compaction triggered mid-run e2e. | inventoried |
| `ResultPersistenceMiddleware` | `core/context_management/result_persistence.py` | Persists large tool outputs and replaces with preview. | (1) Large content truncated; (2) Retrieve via `retrieve_result`. | Persist failure; retrieve on expired key. | `test_integration_features.py` | None. | Full persist→retrieve round-trip e2e. | inventoried |
| `ExtractionMiddleware` | `core/memory/extraction_middleware.py` | Auto-extract memories from turn via an LLM worker. | (1) Agent didn't write memory itself; (2) Skip when agent already wrote; (3) Non-blocking on LLM failure. | LLM failure; empty messages. | `test_extraction.py`, `test_memory.py` | None (triggers background LLM in every e2e; best handled via scripted worker response). | Extraction-reached-store e2e. | inventoried |
| `EmptyTurnMiddleware` | `core/resilience/` | Prevents spinning on empty LLM output. | (1) Nudge up to N times; (2) Exit on final empty. | Per-turn nudge count. | `test_resilience.py` | None. | Empty-output nudge e2e. | inventoried |
| `CompletionGuardMiddleware` | `core/resilience/` | Ensure minimum tool calls before completion. | (1) Min-1 default; (2) Allow direct-answer mode. | Threshold overrides. | `test_resilience.py` | None. | Early-exit nudge e2e. | inventoried |
...
| `PostRunBackstopMiddleware` | `core/resilience/post_run.py` | Absolute backstop — records run metadata. | (1) Normal completion; (2) Error completion; (3) Interrupted run. | Metadata persist failure. | `test_resilience.py` | (indirect — run_metadata appears in MockStore in Phase 3.2). | Metadata record shape assertion e2e. | inventoried |
...
| Feature | Source | Purpose | Main use cases | Edge cases | Unit coverage | E2e coverage | Gaps | Status |
|---|---|---|---|---|---|---|---|---|
| `PersistentMemoryManager` | `core/memory/persistent.py` | CRUD + search facade over a LangGraph Store. | (1) create; (2) get; (3) update; (4) delete; (5) search; (6) list_by_scope. | Unknown id; wrong scope; store failure. | `test_memory.py` | (indirect via Phase 3.2) | Each CRUD op full round-trip e2e. | inventoried |
...
| `AutoMemoryExtractor` | `core/memory/extraction.py` | LLM-driven memory extraction from recent turns. | (1) Emit memory candidates; (2) Skip when agent wrote; (3) Swallow LLM errors. | Parse failures; rate limits. | `test_extraction.py` | Partial (runs in background on every e2e). | Positive case: extraction emits record, asserted in store. | inventoried |
...
| Feature | Source | Purpose | Main use cases | Edge cases | Unit coverage | E2e coverage | Gaps | Status |
|---|---|---|---|---|---|---|---|---|
| `PressureMonitor` | `core/context_management/pressure.py` | Compute signals about context window use. | (1) Fresh state low; (2) Long conversation high. | Empty messages. | `test_context_management.py` | None. | Real multi-turn triggers compaction e2e. | inventoried |
| Compaction strategies | `core/context_management/compaction.py` | Microcompact, summarize, drop-prefix. | (1) Tool-result truncation; (2) Summary of older turns. | Nothing to compact. | `test_pressure_full_compaction.py` | None. | End-to-end under pressure e2e. | inventoried |
...
| `build_coding_agent` | `graphs/coding_agent.py` | Reference + coding workers + git context. | (1) File edit flow. | — | `test_coding_features.py` | None. | Happy path e2e. | inventoried |
...
| `ConversationRecorder` | `replay/recorder.py` | Capture real LLM traffic to a JSON file. | (1) Record-live; (2) Save/load. | Schema drift in `LLMInteraction`. | `test_integration_features.py` | None. | Record → replay round-trip e2e. | inventoried |
| `ReplayAssertions` | `replay/assertions.py` | Structural assertions over recordings. | (1) Tool sequence match; (2) Message count match. | Drift between recording and live. | `test_integration_features.py` | None. | — | inventoried |
| Streaming | `streaming.py` | `astream_events` helpers and filters. | (1) Token stream; (2) Internal-event suppression. | Back-pressure; aborted stream. | `test_streaming.py` | None. | ainvoke → astream parity e2e. | inventoried |

## Cluster J — Integrations

| Feature | Source | Purpose | Main use cases | Edge cases | Unit coverage | E2e coverage | Gaps | Status |
|---|---|---|---|---|---|---|---|---|
| FastAPI contrib | `contrib/fastapi.py` | Expose kit graphs as FastAPI routes. | (1) Mount a graph; (2) Request → ainvoke adapter. | Malformed request; streaming. | `test_contrib_fastapi.py` | None. | End-to-end HTTP round-trip (via TestClient) e2e. | inventoried |
| A2A protocol | `contrib/a2a.py` | Agent-to-agent request wrapping. | (1) Invoke; (2) Stream. | Invalid payload. | `test_integration_features.py` | None. | Wrapper round-trip e2e. | inventoried |
...
| Supervisor pattern | `graphs/supervisor_agent.py` | Multi-agent supervisor. | (1) Dispatch to sub-agent; (2) Aggregate responses. | Sub-agent failure. | `test_integration_features.py` | None. | Dispatch round-trip e2e. | inventoried |
...
| CLI scaffolder | `cli.py` | `python -m langgraph_kit.cli new <name>` — scaffolds new agent. | (1) Emit a working agent; (2) Emitted agent imports and invokes cleanly. | Template drift from kit API. | — | None. | Scaffold + import + smoke-invoke e2e. | inventoried |
