...
**Status as of 2026-04-23:** Phase 4 substantially advanced — Clusters A (memory, skills, UI), B (ToolError, ResultPersist, EmptyTurn), C (all built-in commands including unknown-passthrough), D (section-only, tool-only, multi-plugin, empty-registry), G (stop-hook blocking vs non-blocking error paths), H (build + runtime recursion override + exhaustion), plus cross-cluster invariants and a basic builder smoke. **442 tests passing** (397 baseline → 442 after 45 new e2e scenarios across 12 files). Remaining coverage gaps deferred to later sessions: Cluster A edges (async tasks, HITL, MCP, retrieve_result round-trip), Cluster B edges (RuntimeState, Queued, Pressure mid-run compaction, Backstop explicit record shape), Cluster E (PersistentMemoryManager CRUD unit gaps), Cluster F (compaction-under-load), Cluster I (record→replay round-trip, streaming parity), Cluster J (FastAPI contrib, supervisor, CLI scaffolder). Next action: pick up any of those when resuming.
...
- [~] **Cluster A — Tool system.** `ToolRegistry`, `ToolCapability`, every standard tool (memory, retrieval, search, skill, async, UI, HITL), `DeferredToolRegistry`, `tool_search`, `call_deferred_tool`, MCP tool adapters. **Partial:** memory (CRUD e2e + invalid-arg recovery), skills (list/read + unknown name), UI tools (all 4), deferred tools (full flow). **Deferred:** async tasks, HITL, MCP, retrieve_result round-trip.
- [~] **Cluster B — Middleware stack.** All 12 middlewares individually + stack-ordering invariants; each gets happy path + empty-input edge case + failure-mode test. **Partial:** CommandMiddleware, ToolLoopGuardMiddleware, ToolErrorMiddleware, ResultPersistenceMiddleware, EmptyTurnMiddleware, StopHooksMiddleware (happy + blocking/non-blocking error paths), ordering (save_memory → hook sees state). **Deferred:** RuntimeStateMiddleware, QueuedInputMiddleware, PressureMiddleware (mid-run compaction), ExtractionMiddleware (positive emit), CompletionGuardMiddleware, PostRunBackstopMiddleware (explicit record shape).
...
- [~] **Cluster G — Resilience.** Loop guards, stop hooks (blocking vs non-blocking), `PostRunBackstopMiddleware`, error recovery, fallback strategies. **Covered:** loop guards (unit + e2e + concurrent-isolation), stop hooks error paths. **Deferred:** backstop record shape, error recovery classification.
- [~] **Cluster H — Builders + recursion.** **Covered:** build_deep_agent, build_reference_deep_agent, build_basic_deep_agent (smoke), recursion limit (build + runtime override + exhaustion), DEFAULT_RECURSION_LIMIT contract. **Deferred:** build_coding_agent.
- [ ] **Cluster I — Replay + streaming.** `RecordedChatModel` covered (used by every e2e) + bind_tools unit test. **Deferred:** `ConversationRecorder` full record→replay round-trip, `ReplayAssertions`, streaming event flow parity.
